...
def labels_to_image_model(labels_shape,
    ...
    for i, channel in enumerate(split):
        ...
        if randomise_res:
            ...
            if buil_distance_maps:
                channel, dist = layers.MimicAcquisition(atlas_res, atlas_res, labels_shape, True)([channel, resolution])
                channels.extend([channel, dist])
            else:
                channel = layers.MimicAcquisition(atlas_res, atlas_res, labels_shape, False)([channel, resolution])
            ...
            if downsample[i]:
                ...
                channel = layers.MimicAcquisition(atlas_res, data_res[i], labels_shape)([channel, resolution])
            channels.append(channel)

    # concatenate all channels back
    image = KL.Lambda(lambda x: tf.concat(x, -1))(channels) if len(channels) > 1 else channels[0]

    # compute cumsum and append them to each image
    if do_cumsum:
        image._keras_shape = tuple(image.get_shape().as_list())
        cumsum = layers.NormalisedCumsum(reverse=[False, False, True], apply_noise=[False, False, True])(image)
        image = KL.Lambda(lambda x: insert_cumsum(x[0], x[1], n_channels, n_dims), name='insert_cumsum')([image, cumsum])
    else:
        cumsum = None
    ...
    if crop_shape != labels_shape:
        ...
        image._keras_shape = tuple(image.get_shape().as_list())
        if do_cumsum:
            [labels, image, cumsum] = layers.RandomCrop(crop_shape)([labels, image, cumsum])
        else:
    ...
    if crop_shape != output_shape:
        ...
        if do_cumsum:
            cumsum = l2i_et.resample_tensor(cumsum, output_shape)
    ...
    if do_cumsum:
        cumsum = KL.Lambda(lambda x: x, name='cumsum_out')(cumsum)
        image = KL.Lambda(lambda x: x[0], name='image_out')([image, labels, cumsum])
    else:
...
def insert_cumsum(channels, cumsum_channels, n_channels, n_dims):

    # split inputs and cumsums along the channel dimension
    input_channels = tf.split(channels, [1] * n_channels, axis=-1)
    list_cumsum_channels = tf.split(cumsum_channels, [1] * (n_channels * n_dims), axis=-1)

    # insert cumsums between input channels
    list_channels = list()
    for i in range(n_channels):
        list_channels += [input_channels[i]] + list_cumsum_channels[n_dims * i: n_dims * (i + 1)]
