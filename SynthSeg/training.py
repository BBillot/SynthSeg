...
def training(labels_dir,
             ...
             checkpoint=None):
    """
    This function trains a U-Net to segment MRI images with synthetic scans generated by sampling a GMM conditioned on
    ...
    distribution. Regrouped labels will thus share the same Gaussian when sampling a new image.
    Should be the path to a 1d numpy array with the same length as generation_labels and contain values
    between 0 and K-1, where K is the total number of classes. Default is all labels have different classes.
    ...
    :param max_res_aniso: If randomise_res is True, we use this to downsample the input volumes to a random LR in
    ...
    to a 1d numpy array. In the multi-modal case, it should be given as a numpy array (or a path) of size (n_mod, n_dims),
    ...
    :param unet_feat_count: (optional) number of feature for the first layer of the U-Net. Default is 24.
