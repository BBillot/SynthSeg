...
def training(labels_dir,
    ...
    :param wandb_log_freq: ("epoch", "batch", or int): if "epoch", logs metrics at the end of each epoch.
    ...
    if wl2_epochs > 0:
        ...
        results = train_model(wl2_model, input_generator, lr, wl2_epochs, steps_per_epoch, model_dir, 'wl2', checkpoint)
...
def train_model(model,
    ...
    # fit
    results = model.fit_generator(generator,
                        epochs=n_epochs,
...
def training_from_options(opts: TrainingOptions) -> tf.keras.callbacks.History:
    """
    ...
    return training(labels_dir, model_dir, **keyword_args)
